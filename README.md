...
For low-latency applications, Turbo can also stream audio chunks:
...
for chunk in model.stream(
    "Hi there, this is Chatterbox Turbo streaming.",
    audio_prompt_path="your_10s_ref_clip.wav",
):
    pcm_bytes = audio_to_pcm_s16le(chunk.audio)
    # Send pcm_bytes to your audio device, websocket, or media pipeline.
```
...
Turbo streaming chunks are not watermarked because Perth watermarking currently operates on complete waveforms. Use `generate()` when the final output must include a watermark.
